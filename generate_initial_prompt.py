...
DEFAULT_PROMPT = '''You are working in a pre-existing TypeScript application. 
...
- Update  to reflect any new files or components.
...
def main():
    ...
    prompt = DEFAULT_PROMPT.format(task_readme_path=task_readme_path, important_llm_docs_path=important_llm_docs_path)
    with open(INITIAL_PROMPT_PATH, "w") as f:
        f.write(prompt)
    print(f"Wrote initial prompt to {INITIAL_PROMPT_PATH}")
